...
int main(){
	...
	for (int i=0;i<n;i++){
		a[i].name=stu[i].name;
		if (stu[i].averpoint>80&&stu[i].nop>=1){ // 院士奖学金
			a[i].awards+=8000;
		}
		if (stu[i].averpoint>85&&stu[i].reviewpoint>80){ // 五四奖学金
			a[i].awards+=4000;
		}
		if (stu[i].averpoint>90){ // 成绩优秀奖
			a[i].awards+=2000;
		}
		if (stu[i].averpoint>85&&stu[i].Isweststu=='Y'){ //西部奖学金
			a[i].awards+=1000;
		}
		if (stu[i].reviewpoint>80&&stu[i].Isleader=='Y'){ // 班级贡献奖
			a[i].awards+=850;
		}
	}
